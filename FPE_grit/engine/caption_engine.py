...
def evaluate_metrics(
    ...
    with tqdm(desc=f'Epoch {epoch} - evaluation on {split}', unit='it', total=len(dataloader)) as pbar:
        ...
        for it, batch in enumerate(iter(dataloader)):
            ...
            batch['samples_freq'] = batch['samples_freq'].to(device)
            counter += 1
            start_it = time.time()
            with torch.no_grad():
                out, _ , _ = model(
                    batch['samples'],
                    seq=None,
                    images_freq=batch['samples_freq'],
                    use_beam_search=True,  # SC
    ...
    with tqdm(desc='Epoch %d - validation' % epoch, unit='it', total=len(dataloader)) as pbar:
        with torch.no_grad():
            for it, batch in enumerate(dataloader):
                ...
                out = model(batch['samples'], batch['captions'], batch['samples_freq'])
...
def train_xe(
    ...
    if scheduler is not None:
        scheduler.step()
    running_loss = .0
    with tqdm(desc=f'Epoch {epoch} - train in XE', unit='it', total=len(dataloaders['train'])) as pbar:
        ...
        for it, batch in enumerate(dataloaders['train']):
            ...
            if use_hdf5_feat:  
                ...
                batch['samples'] = batch['samples'].to(device)
                batch['captions'] = batch['captions'].to(device)
                batch['samples_freq'] = batch['samples_freq'].to(device)   #  frequency input  !!!!


            
            out = model(batch['samples'], batch['captions'], batch['samples_freq'])   #  (b_s, seq_len, viocabulary_token)
...
def train_sc(model,
    ...
    with tqdm(desc='Epoch %d - train in SC' % epoch, unit='it', total=len(dataloaders['train_dict'])) as pbar:
        for it, batch in enumerate(dataloaders['train_dict']):
            ...
            outs, log_probs, _ = model(
                batch['samples'],   # images
                seq=None,
                images_freq=batch['samples_freq'],  # frequency
                use_beam_search=True,
