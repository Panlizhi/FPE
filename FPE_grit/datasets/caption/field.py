    ...
    def preprocess(self, path, image_id=None):
        ...
        if self.use_hdf5_feat:
            ...
            if self.transform is not None:
                # Train    resize, RandAugment(), ToTensor(), normalize() ×
                # Train    resize, ToTensor(), normalize() √
                # Valid    resize , ToTensor(),  normalize()
                img_trans = self.transform(img)       # [3, 384, 640]


        #  image for GRIT [384, 640]    
        return img_trans
