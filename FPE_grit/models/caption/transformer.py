...
class Transformer(BaseCaptioner):
    ...
    def __init__(self, detector, config=None):
        ...
        self.use_freq_feat = config.model.use_freq_feat
        ...
        if self.use_freq_feat:
            # self.gamma = config.model.freq_net.gamma
            self.register_state('samples_freq_img', None)  
            self.register_state('samples_freq_mask', None)  
    ...
    def forward(self,
        ...
        if not use_beam_search:    
            ...
            if self.config.model.use_gri_feat:
                # [b, 3, (h w)=60, 512]  d_model   <---  [16, 60, 1024]
                gri_feat, _ = self.grid_net(vis_inputs['gri_feat'], vis_inputs['gri_mask'])
                # [b, (h w)=60, 512]  
                vis_inputs['gri_feat'] = gri_feat[:, -1]

            #                                                     add frequency images
            dec_output, _  = self.cap_generator(seq, vis_inputs, images_freq.tensors, images_freq.mask)
    ...
    def step(self, timestep, prev_output, samples,  samples_freq, seq, mode='teacher_forcing', **kwargs):
        
        it = None
        freq_tensors = freq_mask=None
        ...
        if mode == 'teacher_forcing':
            ...
            if timestep == 0:
                if not self.cached_features:
                    ...
                    # images.tensors.shape: in detector.py torch.Size([16, 3, 384, 640])
                else:
                    vis_inputs = samples
                ...
                #   (bs, 1) ，所有值都填充为 bos_idx。
                it = _feat.data.new_full((_feat.shape[0], 1), self.bos_idx).long()

                if self.config.model.use_freq_feat:
                    self.samples_freq_img = samples_freq.tensors
                    self.samples_freq_mask = samples_freq.mask

            else:
                it = prev_output  #  (bs* beam, 1)  
                

        vis_inputs = {}
        if self.config.model.use_gri_feat:
            # 用self.apply_to_states 实现beam扩展
        ...
        if self.config.model.use_freq_feat:
            freq_tensors=self.samples_freq_img
            freq_mask=self.samples_freq_mask


        return self.cap_generator(it, vis_inputs, freq_tensors, freq_mask)      
    ...
    def iter(self, timestep, samples, samples_freq, outputs, return_probs, batch_size, beam_size=5, eos_idx=3, **kwargs):
        cur_beam_size = 1 if timestep == 0 else beam_size 

        word_logprob, atten_weights_list = self.step(timestep, self.selected_words, samples, samples_freq, None, mode='feedback', **kwargs)
        word_logprob = word_logprob.view(batch_size, cur_beam_size, -1)  # [BB Vicabu] -> [B Beam V]
        ...
        outputs.append(repeat(selected_words, 'B Beam -> B Beam L', L=1)) # 词 拼接到 句子
