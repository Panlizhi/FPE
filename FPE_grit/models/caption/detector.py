...
class Detector(nn.Module):
    ...
    def forward(self, images: NestedTensor):
        ...
        mask = images.mask  # padding mask [B, H, W]   原始模板


        # print("images.tensors.shape: in detector.py", images.tensors.shape)

        # features = [[B, C1, H1, W1], [B, C2, H2, W2], [B, C3, H3, W3], [B, C4, H4, W4]]
        features = self.backbone(x)

        # swin的输出
        # 经过3个grid layer 获得 gri_feat
        # 经过6个det_module 获得 reg_feat


        # 将 mask 的空间尺寸调整为当前特征层 f 的高和宽（f.shape[-2:]）。
        masks = [
            F.interpolate(mask[None].float(), size=f.shape[-2:]).to(torch.bool)[0] for l, f in enumerate(features)
        ]  # masks [[B, Hi, Wi]]    特征获取后的模板插值
        ...
        outputs['gri_feat'] = rearrange(features[-2], 'b c h w -> b (h w) c')
        outputs['gri_mask'] = repeat(masks[-2], 'b h w -> b 1 1 (h w)')

        # print("masks[-2]  in detector.py",masks[-2][0,:,:])
        # print("masks[-2].shape  in detector.py", masks[-2].shape)
...
def build_detector(config):
    ...
    if os.path.exists(config.model.detector.checkpoint):
        ...
        # missing: 当前模型中存在但权重文件中没有的层（会被随机初始化） unexpected: 权重文件中有但当前模型不存在的层（会被忽略）
        # 可以灵活地重用预训练模型的部分权重
        print(f"Loading weights for detector (swin+De-DETR): missing: {len(missing)}, unexpected: {len(unexpected)}.")
    return detector
