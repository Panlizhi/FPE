    ...
    def forward(self, q, k, v, attention_mask=None):
        ...
        if self.n_memories > 0:
            ...
            if attention_mask is not None:
                scores[:, :, :, :nk] = scores[:, :, :, :nk].masked_fill(attention_mask.bool(), -np.inf)  # 将 attention_mask 为 True 的位置替换为 -np.inf（负无穷）
        else:
            q = rearrange(self.fc_q(q), 'b nq (head d) -> b head nq d', head=self.n_heads)
...
class Attention_fpe(nn.Module):
    ...
    def forward(self, q, k, v, attention_mask=None):
        ...
        def print_stats(tensor, name):
            print(f"{name} | min: {tensor.min():.6f}, max: {tensor.max():.6f}, mean: {tensor.mean():.6f}")
        
        # print_stats(p_attn, "p_attn output")
        # [b h nq nk] * [b h nk dv] = [b h nq dv] -> [b nq h dv] -> [b nq h*dv]
        out = rearrange(torch.matmul(p_attn, v), 'b h nq dv -> b nq (h dv)')
        # print_stats(torch.matmul(p_attn, v), "torch.matmul(p_attn, v) output")
        # print_stats(out, "out111 output")
        out = self.fc_o(out)  # (b_s, nq, d_model)
        # print_stats(out, "out222 output")

        return out


class Attention_w_weights(nn.Module):
    '''
    Scaled dot-product attention
    '''
    ...
    def forward(self, q, k, v, attention_mask=None):
        ...
        if self.n_memories > 0: # memory bank
            ...
            if attention_mask is not None:
                # scores位置中对应于attention_mask位置为True的元素被替换为 -np.inf
                #      [b h nq nk]        [b h nq nk]   (b_s, 1, seq_len, seq_len)
                scores = scores.masked_fill(attention_mask.bool(), -np.inf) 
...
class MultiHeadAttention_wo_residual(Module):
    ...
    def __init__(self, d_model, n_heads, dropout=.1, n_memories=0, can_be_stateful=False):
        ...
        self.can_be_stateful = can_be_stateful
        if self.can_be_stateful:  # store prev computed K & V for fast inference
            self.register_state('running_keys1', torch.zeros((1, d_model)))
            self.register_state('running_values1', torch.zeros((1, d_model)))
            self.register_state('running_features1', torch.zeros(1, d_model))  # 收集生成序列的features
            
    
    def forward(self, queries, keys, values, attention_mask=None, pad_mask=None):
        if self.can_be_stateful and self._is_stateful:
            # keys, values:             from the current input token: [B, 1, D]
            # running_keys, values:     from prev tokens: [B, t-1, D]
            self.running_keys1 = torch.cat([self.running_keys1, keys], 1)
            self.running_values1 = torch.cat([self.running_values1, values], 1)
            
            if self.timestep == 0:
                keys = self.running_keys1 = self.running_keys1[:, 1:]  # [B t D]
                values = self.running_values1 = self.running_values1[:, 1:]  # [B t D]
            else:
                keys = self.running_keys1  # [B t D]
                values = self.running_values1  # [B t D]

        out = self.attention(queries, keys, values, attention_mask) * pad_mask    # XE 句子特征  SC 单词特征
        
        if self.can_be_stateful and self._is_stateful:
            #                              prev  out [B, t-1, D]  current out: [B, 1, D]
            self.running_features1 = torch.cat([self.running_features1, out], 1)  # [Bs*Beam, step, d_model]  更新features缓存
            if self.timestep == 0:
                self.running_features1 = self.running_features1[:, 1:] # 去除初始的0特征

            self.timestep += 1
            return self.running_features1 #  SC 句子特征 
        else: 
            return out    # XE 句子特征


class MultiHeadAttention_w_weights(Module):

    def __init__(self, d_model, n_heads, dropout=.1, n_memories=0, can_be_stateful=False):
        super().__init__()

        self.attention = Attention_w_weights(d_model=d_model, n_heads=n_heads, dropout=dropout, n_memories=n_memories)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
